...
void main()
{
    ...
    const i32vec2 alloc_request_page_coords = deref(_vsm_allocation_buffer[id]).coords;
    ...
    if(id < header.free_buffer_counter)
    {
        ...
        imageStore(daxa_uimage2D(_vsm_page_table), alloc_request_page_coords, u32vec4(new_vsm_page_entry));
    ...
    else if (free_shifted_id < header.not_visited_buffer_counter)
    {
        ...
        const i32vec2 owning_vsm_coords = get_vsm_coords_from_meta_entry(meta_entry);
        imageStore(daxa_uimage2D(_vsm_page_table), owning_vsm_coords, u32vec4(0));
        // debugPrintfEXT("owning vsm page table coords %d, %d\n", owning_vsm_coords.x, owning_vsm_coords.y);
    ...
    {
        imageStore(daxa_uimage2D(_vsm_page_table), alloc_request_page_coords, u32vec4(allocation_failed_mask()));
    }
}
