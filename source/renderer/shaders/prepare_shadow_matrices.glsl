...
void main()
{
    ...
    light_camera_rotation[0] = normalize(cross(f32vec3(0.0, 1.0, 0.0), light_camera_rotation[2]));
    ...
    deref(_shadowmap_matrices[thread_idx]).cascade_matrix = shadow_proj_view;
    ...
    if(deref(_globals).use_debug_camera)
    {
        ...
        if(thread_idx == 0 || thread_idx == 1 || thread_idx == 2 || thread_idx == 3)
        {
            ...
            f32mat4x4 inverse_shadow_proj_view = inverse(shadow_proj_view);
