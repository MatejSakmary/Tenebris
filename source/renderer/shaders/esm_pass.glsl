...
shared f32 depth_lds [WORKGROUP_SIZE + 4];

f32 get_depth(i32 local_thread_index, i32 offset)
{
    // add two because that is the left overhang of the 5 wide gaussian kernel
    const i32 index = local_thread_index + offset + 2;
...
void main()
{
    ...
    u32vec2 wg_image_coords;
    u32vec2 image_coords;
    ...
    // const i32vec2 offset_image_coords = i32vec2(image_coords + pc.offset);
    for(i32 wg_offset = 0; wg_offset < 2 * WORKGROUP_SIZE; wg_offset += WORKGROUP_SIZE)
    {
    ...
    for(i32 wg_offset = 0; wg_offset < 2 * WORKGROUP_SIZE; wg_offset += WORKGROUP_SIZE)
    {
        ...
        if(gl_LocalInvocationIndex + wg_offset < WORKGROUP_SIZE + 4)
        {
            ...
            const f32 d0 = imageLoad(daxa_image2DArray(_esm_tmp_map), sample_coords).r;
            depth_lds[gl_LocalInvocationIndex + wg_offset] = d0;
        }
    }
#endif
...
#endif // SECOND_PASS
        i32vec3(image_coords, pc.cascade_index),
