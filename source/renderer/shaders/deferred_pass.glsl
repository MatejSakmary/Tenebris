...
void main() 
{
    ...
    if(normal.xyz == f32vec3(0.0, 0.0, 0.0))
    {
        ...
        f32vec2 remap_uv = (uv * 2.0) - 1.0;
        // TODO(msakmary) don't calculate inverse matrix in every fragment
        f32vec4 unproj_pos = inverse(deref(_globals).projection) * f32vec4(remap_uv, depth, 1.0);
        f32vec3 view_pos = -unproj_pos.xyz / unproj_pos.w;
        ...
        u32 cascade_idx = 0;
        // Figure out the cascade this fragment falls into
        for(cascade_idx; cascade_idx < NUM_CASCADES; cascade_idx++)
        {
            if(view_pos.z < deref(_cascade_data[cascade_idx]).cascade_far_depth)
            {
                break;
            }
        }
